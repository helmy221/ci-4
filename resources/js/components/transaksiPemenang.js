window.TransaksiPemenangUI = function() {
    return {
        users: [],
        roles: [],
        units: [],
    ...
    paketList: [],
        ...
        async openEditModalPemenang(data) {
            this.editForm = JSON.parse(JSON.stringify(data));
            // load paket list for the selected pemenang via AJAX
            await this.loadPaketListForPemenang(this.editForm.pemenang);
            this.showEditModalPemenang = true;
        },

        async loadPaketListForPemenang(pemenang) {
            this.paketList = [];
            if (!pemenang) return;
            try {
                const q = new URLSearchParams({ pemenang: pemenang });
                const res = await fetch(`/transaksi/pemenang/list?${q.toString()}`, {
                    method: 'GET',
                    headers: { 'Accept': 'application/json' }
                });
                if (!res.ok) {
                    console.error('Failed to fetch paket list', res.status);
                    return;
                }
                const json = await res.json();
                if (json.status === 'success') {
                    this.paketList = json.data || [];
                } else {
                    this.paketList = [];
                }
            } catch (err) {
                console.error('Error loading paket list:', err);
                this.paketList = [];
            }
        },

        async closeEditModal() {
            this.showEditModalPemenang = false;
            this.editForm = {};
            this.selectedRoles = [];
            this.selectedUnit = '';
